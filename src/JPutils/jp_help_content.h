...
namespace jp_help
{
	...
	inline const std::vector<Line> &table()
	{
		...
		E("Brush property", "Click the SIZE label to choose size, opacity, hardness or stabilizer; modifiers remain quick temporary overrides. Stylus/touch pressure controls point width when the backend provides it",
			"Clic en la etiqueta TAM para elegir tamano, opacidad, dureza o estabilizador; los modificadores siguen siendo atajos temporales. La presion de lapiz/touch controla el ancho cuando el sistema la informa",
			Scope::Paint),
		E("Ctrl+Z", "Undo. Add Shift to redo",
			"Deshacer. Con Shift, rehacer", Scope::Paint),
		E("Ctrl+J / Ctrl+E", "Duplicate the active layer / merge it down (normal, opaque, visible, unlocked layers)",
		...
		E("Ctrl+Shift+P", "Export the current cel as PNG",
			"Exportar el cuadro actual como PNG", Scope::Paint),
		E("Ctrl+Alt+P", "Export every cel as a numbered PNG sequence",
			"Exportar todos los cuadros como secuencia PNG numerada", Scope::Paint),
		E("Ctrl+Shift+G", "Export an animated GIF using FPS and each cel's hold duration",
