...
namespace jp_help
{
	...
	inline const std::vector<Line> &table()
	{
		...
		E("Ctrl/Cmd+G", "Group the selected boxes into one box-group. The boxes move in alive: nothing is recompiled and nothing is reloaded",
			"Agrupar las cajas seleccionadas en un grupo. Las cajas se mueven vivas: no se recompila ni se recarga nada", Scope::Nodes),
		E("Ctrl/Cmd+Shift+G", "Ungroup: dissolve every selected group back into this view, as one undo step. The boxes land where the group stood and move out alive, so nothing is recompiled or reloaded",
			"Desagrupar: disuelve todos los grupos seleccionados de vuelta en esta vista, en un solo paso de deshacer. Las cajas quedan donde estaba el grupo y salen vivas, asi que no se recompila ni se recarga nada", Scope::Nodes),
		E("Ctrl/Cmd+Z", "Undo the last graph edit. Add Shift to redo. Covers moving, deleting, connecting, grouping, parameter values, box state, the active render, and grouping in both directions. One history for the whole composition: a step taken inside a group is undone from anywhere, and the canvas goes there to show it. The mapping panel keeps its own. Adding a box, MIDI, zoom and pan are not recorded",
			"Deshacer la ultima edicion del grafo. Con Shift, rehacer. Cubre mover, borrar, conectar, agrupar, valores de parametros, estado de cajas, la caja activa del render, y agrupar y desagrupar. Un solo historial para toda la composicion: un paso hecho dentro de un grupo se deshace desde donde estes, y el canvas te lleva ahi para que lo veas. El panel de mapeo lleva el suyo aparte. Agregar una caja, MIDI, zoom y paneo no se registran", Scope::Nodes),
		...
		E("H / Shift+H", "Flip the selection horizontally / vertically",
			"Voltear la seleccion horizontal / verticalmente", Scope::Paint),
		...
		E("Ctrl/Cmd+J / Ctrl/Cmd+E", "Duplicate the active layer / merge it down (normal, opaque, visible, unlocked layers)",
			"Duplicar la capa activa / combinarla hacia abajo (capas normales, opacas, visibles y desbloqueadas)", Scope::Paint),
		...
		E("Ctrl/Cmd+Shift+P", "Export the current cel as PNG",
			"Exportar el cuadro actual como PNG", Scope::Paint),
		E("Ctrl/Cmd+Alt+P", "Export every cel as a numbered PNG sequence",
			"Exportar todos los cuadros como secuencia PNG numerada", Scope::Paint),
		E("Ctrl/Cmd+Shift+G", "Export an animated GIF using FPS and each cel's hold duration",
			"Exportar GIF animado usando los FPS y la duración de cada cuadro", Scope::Paint),
