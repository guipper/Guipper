...
namespace jp_help
{
	...
	inline const std::vector<Line> &table()
	{
		...
		E("1 - 7",
			"Switch screens: 1 NODES, 2 SETTINGS, 3 HELP, 4 IMPORT, 5 EDITOR, 6 MIDI, 7 AUDIO",
			"Cambia de pantalla: 1 NODES, 2 SETTINGS, 3 HELP, 4 IMPORT, 5 EDITOR, 6 MIDI, 7 AUDIO", Scope::Global, "1 2 3 4 5 6 7"),
		N("The top bar provides the same seven screens as buttons. CUE and MAP form a separate group for staging and projection mapping.",
			"La barra superior ofrece las mismas siete pantallas como botones. CUE y MAP forman un grupo separado para preparación y mapping de proyección."),
		E("CUE",
		...
		N("Kick and Snare get Sens and Hold instead: Sens is how far the flux must rise above its own moving average, Hold is the minimum time between two detections. The plot shows the flux against that threshold, which is how you see WHY a hit did not register.",
			"Kick y Snare reciben en cambio Sens y Hold: Sens es cuánto debe superar el flujo su propio promedio móvil, y Hold el tiempo mínimo entre dos detecciones. El gráfico muestra el flujo contra ese umbral, que es como se ve POR QUÉ un golpe no registró.", Scope::Settings),
		N("If everything sits pinned at 1.0, look at the normaliser's floor and peak on each meter: when they converge the band has become a comparator. The INPUT meter with its gate mark answers whether any signal is arriving at all.",
			"Si todo queda clavado en 1.0, mira el piso y el pico del normalizador en cada medidor: cuando se juntan, la banda se convirtió en un comparador. El medidor INPUT con su marca de compuerta responde si está entrando señal.", Scope::Settings),
		...
		N("With a parameter on a pattern (ping-pong, random, forward, reverse), the small audio button next to the speed knob makes AUDIO DRIVE THE SPEED. The knob becomes the ceiling: silence stops the pattern, a hit runs it at the knob's rate.",
			"Con un parámetro en un patrón (ida y vuelta, aleatorio, adelante, atrás), el botón chico de audio al lado de la perilla de velocidad hace que el AUDIO MANEJE LA VELOCIDAD. La perilla pasa a ser el techo: en silencio el patrón se frena, y con un golpe corre a la velocidad que marca."),
		N("BPM sync has the same Shaping button, with Amount, Curve, Release and Invert. Curve bends the decay, Invert turns it into a ramp INTO the beat instead of away from it, Release extends the tail and Amount sets the depth. Release only slows the FALL - the rise on the beat stays instant, which is what makes it a pulse.",
			"El sync por BPM tiene el mismo botón Shaping, con Amount, Curve, Release e Invert. Curve dobla la caída, Invert la convierte en una rampa HACIA el golpe en vez de alejarse de él, Release alarga la cola y Amount marca la profundidad. Release solo frena la CAÍDA: la subida en el golpe sigue siendo instantánea, que es lo que lo hace un pulso."),
		N("There is no Threshold or Attack there: the beat envelope already starts at full, so gating its bottom would only clip the tail, and its rise is instant by design.",
			"Ahí no hay Threshold ni Attack: la envolvente del pulso ya arranca en el máximo, así que recortarle el piso solo cortaría la cola, y su subida es instantánea por diseño."),
		N("The Down/Up/Centre chip picks which way it moves: Down makes the knob a ceiling (silence slows it), Up makes it a floor (audio pushes faster), Centre makes it the middle and the audio swings both ways. Right click steps backwards.",
			"El chip Down/Up/Centre elige hacia dónde se mueve: Down hace de la perilla un techo (el silencio lo frena), Up la hace un piso (el audio acelera por encima), y Centre la pone al medio y el audio se mueve para los dos lados. El clic derecho retrocede."),
		N("Amount is the depth: 1 uses the full range, 0 bypasses the modulator and the knob rules on its own.",
			"Amount es la profundidad: en 1 usa todo el recorrido, y en 0 desconecta el modulador y manda solo la perilla."),
		N("It reuses that parameter's own source and shaping, and falls back to the plain knob speed when no audio is running, so turning the input off does not freeze the composition.",
			"Reusa la fuente y el shaping del propio parámetro, y si no hay audio corriendo vuelve a la velocidad de la perilla, así apagar la entrada no congela la composición."),
