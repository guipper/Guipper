...
namespace jp_help
{
	...
	inline const std::vector<Line> &table()
	{
		...
		E("g", "Fill: click a region to flood it. The size slider becomes the tolerance while this tool is selected",
			...
			Scope::Paint),
		E("s", "Free select (lasso): clips without deforming and selects the inside; drag to move, use the top handle to rotate or corners to scale. DEL removes it, D or Alt+drag duplicates it",
			"Seleccion libre (lazo): recorta sin deformar y selecciona el interior; arrastra para mover, usa la manija superior para rotar o las esquinas para escalar. DEL borra, D o Alt+arrastrar duplica",
			Scope::Paint),
