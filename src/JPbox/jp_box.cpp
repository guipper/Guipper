...
void JPbox::draw() {
  ...
  ofSetColor(0);
  float sepsize =
      10; // SEPARACION ENTRE LA LINEA Y LA CAJA Y LA ALINEACION DEL TEXTO.
  ...
  jp_constants::p_font.drawString(shortname, x - width / 2 + sepsize,
  ...
  ofSetColor(255);
  if (outletActiveFlag) {
    ofSetColor(ofColor(0, 255, 0).getLerped(
        ofColor(255), sin(ofGetElapsedTimeMillis() * 0.01) * .5 + .5));
...
void JPbox::draw_outlet() {
  ...
  float trianglesize =
      outlet_size * 5.0; // ESTO LO MODIFICO ACA PARA NO MODIFICAR EL DRAGOBJECT
  ...
  float gotasize = 0.2;
  jp_constants_img::outlet_img.draw(
      0, 0, jp_constants_img::outlet_img.getWidth() * gotasize,
...
int JPbox::getTipo() { return tipo; }
void JPbox::setonoff(bool _val) {
  onoff.boolValue = _val;
  onoff.value = _val;
}
bool JPbox::getonoff() { return onoff.boolValue; }
