...
JPbox *JPboxgroup::addBox(string directory, float _x, float _y)
{
	...
	if (isGroupViewActive())
	{
		...
		if (preset != nullptr)
		{
			...
			JPbox *bx = jp_box_factory::create(directory, jp_box_factory::Context::Interactive);
			if (bx == nullptr)
			{
				return nullptr;
			}
...
void JPboxgroup::copySelectedBoxes()
{
	...
	for (int si : srcIndices)
	{
		...
		// Parameters
		jp_parameter_xml::save(data, box->parameters);
...
void JPboxgroup::pasteBoxes()
{
	...
	for (auto &box : boxloader)
	{
		...
		JPbox *bx = jp_box_factory::create(dir, jp_box_factory::Context::Interactive);
		...
		jp_parameter_xml::load(box, bx->parameters,
			jp_parameter_xml::LoadContext::Clipboard);
