...
void JPboxgroup::draw() {
  ...
  /*if (draw_SelectionRect) {
          ...
          ofNoFill();
          ofSetLineWidth(5);
          ofVec2f center = ofVec2f((ofGetMouseX()+ lastMouseClick.x)/2,
  (ofGetMouseY()+ lastMouseClick.y)/2); float w = abs(ofGetMouseX() -
  lastMouseClick.x) ; float h = abs(ofGetMouseY() - lastMouseClick.y) ;
          ofDrawRectangle(center.x, center.y,w, h);
          ofSetColor(0, 0, 255);
          ofDrawLine(ofGetMouseX(), ofGetMouseY(), lastMouseClick.x,
  lastMouseClick.y); ofSetRectMode(OF_RECTMODE_CORNER);
  }*/
  ...
  for (int i = 0; i < boxes.size(); i++) {
    ...
    if (*activerender == i) {
      ...
      ofRectMode(CENTER);
      ofRectRounded(x, y - boxes[i]->height / 2 + 10, boxes[i]->width * 1.1,
                    boxes[i]->height * 1.1, 10);
    }
    boxes[i]->draw();
...
void JPboxgroup::draw_paramswindow() {
  ...
  if (openguinumber != -1 && boxes.size() > 0) {
    ...
    ofSetRectMode(OF_RECTMODE_CENTER);
    ofSetColor(255, 255);
    ofDrawRectangle(inspectorwindow_x, inspectorwindow_y, inspectorwindow_width,
    ...
    */
    ofSetColor(120, 255);
    // constants_img::background.draw(inspectorwindow_x, inspectorwindow_y,
    ...
    jp_constants::h_font.drawString(
        name, inspectorwindow_x - jp_constants::h_font.stringWidth(name) / 2,
        ...
        if (boxes[k]->fbohandlergroup.getFboName(l) == boxes[i]->name) {
          ...
          if (boxes[i]->outletActiveFlag) {
            ...
            if (boxes[k]->fbohandlergroup.getSize() > 0) {
              boxes[i]->triangleangle = atan2(
                  boxes[k]->fbohandlergroup.getPosY(l) - boxes[i]->outlet_y,
                  boxes[k]->fbohandlergroup.getPosX(l) - boxes[i]->outlet_x);
            } else {
              boxes[i]->triangleangle =
                  atan2(ofGetMouseY() - boxes[i]->outlet_y,
                        ofGetMouseX() - boxes[i]->outlet_x);
            }
          }
          ...
          ofDrawLine(boxes[k]->fbohandlergroup.getPosX(l),
                     ...
                     boxes[i]->outlet_x + boxes[i]->outlet_size / 2,
                     boxes[i]->outlet_y);
        } else {
        }
      }
...
void JPboxgroup::update() {
  ...
  for (int i = boxes.size() - 1; i >= 0; i--) {
    ...
    // ESTO ES PARA QUE EL SLIDER QUE REPRESENTA LA BARRA DE TIEMPO DE LOS
    // VIDEOS SE ACTUALICE
    ...
    if (ofGetMousePressed()) {
      if (boxes[i]->mouseOverOutlet() && !ouletagarrado && !shaderboxagarrado) {
        ...
        cualestaagarrado = -1;
      } else if (boxes[i]->mouseOver() && !ouletagarrado &&
                 !shaderboxagarrado) {
        cualestaagarrado = i;
        outlet_cualestaagarrado = -1;
        ouletagarrado = false;
    ...
    if (!jp_constants::systemDialog_open &&
      ...
      if (ofFile(boxes[i]->dir).exists()) {
        ...
        if (lasttimemodified != boxes[i]->datemodified) {
          ...
          // UF ESTO ESTA ATADO CON ALAMBRE MUY FUERTE. ACA HAY UN BUG QUE LO
          // QUE HACE ES QUE NO RECARGUE BIEN EL SHADER. BASICAMENTE LO QUE
          // SUCEDE ES QUE CUANDO VOLVES A CARGAR Y GUARDAR A VECES NO LEVANTA
          // LOS PARAMETROS ENTONCES LE DIGO QUE LO REINICIE HASTA QUE LA
          // CANTIDAD DE PARAMETROS SEA COMO LA CORRECTA DIGAMOS. OSEA
          // TECNICAMENTE EXISTE LA POSIBILIDAD 0.00000000000000000001% DE QUE
          // NUNCA CARGUE BIEN Y ENTRE EN UN LOOP INFINITO DE MUERTE Y
          // DESTRUCCION. OSEA AHORA AL MENOS CARGA BIEN SIEMPRE. LO QUE NO
          // PUEDO HACER ES QUE ME VUELVA A CARGAR LOS VALORES QUE TENIA CON LOS
          // RENDER QUE TENIA.

          // cout << "Parameter a size :" << boxes[i]->parameters.getSize() <<
          // endl;
          JPbox *aux;

          boxes[i]->reload();
          cout << "RElOAD SHADER" << endl;
          // cout << "Parameter d size :" << boxes[i]->parameters.getSize() <<
          // endl;
  ...
  // ESTO HABLA DE LO MAL QUE PROGRAMAS : MIRA MIRA LO QUE ES ESTO SE FUE A LA
  // MEIRDA EL CODIGO : LA VARIABLE NEEDSUPDATE DETERMINA SI EL BOTON FUE
  // APRETADO Y SI NECEITA ACTUALIZARSE LO HACE Y LA VUELVE A SETEAR A FALSE ACA
  // LO QUE PASA ES QUE ESTO SOLUCIONA EL TEMITA DEL SYNC CUANDO SE DESPLIEGA EL
  // BOTON PARA QUE NO QUEDE EN CUALQUIERA OSEA EL NEEDSUPDATE COMUNICA QUE EL
  // BOTON DEL COLLAPSE DE LOS SLIDERS FUE APRETADO.
  if (openguinumber != -1) {
  ...
  // activeSequence = true;
  if (activeSequence &&
      ofGetElapsedTimeMillis() - lasttime_sequence >
          jp_constants::durationgallery &&
      boxes.size() > 2) {
    lasttime_sequence = ofGetElapsedTimeMillis();
...
void JPboxgroup::setinspectorsetactiveparams() {
  ...
  inspectorsetactive.setup(
      ofGetWidth() - inspectorwindow_width * 1 / 4, inspectorwindow_height,
      inspectorwindow_setactivesize * 2., inspectorwindow_setactivesize);

  inspectorwindow_height += inspectorwindow_sepy;
  inspectorreload.setup(
      ofGetWidth() - inspectorwindow_width * 1 / 4, inspectorwindow_height,
      inspectorwindow_setactivesize * 2., inspectorwindow_setactivesize);
...
void JPboxgroup::update_mouseDragged(int mousebutton) {
  ...
  if (cualestaagarrado != -1 && boxes[cualestaagarrado]->activeFlag) {
    float distx =
        boxes[cualestaagarrado]->x - (ofGetPreviousMouseX() - ofGetMouseX());
    float disty =
        boxes[cualestaagarrado]->y - (ofGetPreviousMouseY() - ofGetMouseY());
  ...
  // Para los sliders :
  if (!shaderboxagarrado && !ouletagarrado && cualestaagarrado == -1 &&
      outlet_cualestaagarrado == -1 && openguinumber != -1) {
    // Con esto detecto que no se toque ningun slider de mas: osea que no puedas
    // estar tocando dos sliders a la vez :
    bool slideragarrado = false;
    // Si invertimos el for en este crashea. habra que cambiarlo en otro lugar
    // tambien?
    for (int i = 0; i < controllers.size(); i++) {
      if (controllers[i]->activeFlag) {
        slideragarrado = true;

        if (boxes[openguinumber]->parameters.getType(i) ==
                boxes[openguinumber]->parameters.FLOAT &&
            boxes[openguinumber]->parameters.getMovType(i) == 0) {
          // float valf = ofLerp(controllers[i]->value,
          // boxes[openguinumber]->parameters.getFloatValue(i), 0.1);
          ...
          // Esto es porque si esta en movtype 0 no actualiza para que no pise
          // con el OSC , entonces hay que actualizarlo manualmente Esta es la
          // actualizaci�n manual. Acordate que si el movtype esta en 0 el valor
          // NO SE ACTUALIZA.
          controllers[i]->value =
              ofMap(ofGetMouseX(),
          ...
          boxes[openguinumber]->parameters.setFloatValue(controllers[i]->value,
                                                         i);
          boxes[openguinumber]->parameters.setFloatLerpValue(
              controllers[i]->value, i);
          // boxes[openguinumber]->parameters.setSpeed(controllers[i]->speed,
          // i); boxes[openguinumber]->parameters.setMin(controllers[i]->min,
          // i); boxes[openguinumber]->parameters.setMax(controllers[i]->max,
          // i);
        }
      }
    }
...
void JPboxgroup::update_mousePressed(int mouseButton) {
  ...
  isDoubleClick =
      (ofGetSystemTimeMillis() - lasttime_mouseclick < duration_mouseclick);
  ...
  if (mouseOverGui() && openguinumber != -1) {
    ...
    for (int i = 0; i < controllers.size(); i++) {
      if (controllers[i]->mouseOver()) {
        ...
        if (boxes[openguinumber]->parameters.getType(i) ==
          ...
          controllers[i]->activeFlag = true;
          activeone = i;
        } else if (boxes[openguinumber]->parameters.getType(i) ==
                   boxes[openguinumber]->parameters.BOOL) {
          boxes[openguinumber]->parameters.setBoolValue(
              controllers[i]->boolValue, i);
        }
      }
      if (boxes[openguinumber]->parameters.getType(i) ==
          boxes[openguinumber]->parameters.FLOAT) {
    ...
    for (int i = 0; i < controllers.size(); i++) {
      if (i != activeone) {
        ...
        // Esto es porque si esta en movtype 0 no actualiza para que no pise con
        // el OSC , entonces hay que actualizarlo manualmente Esta es la
        // actualizaci�n manual. Acordate que si el movtype esta en 0 el valor
        // NO SE ACTUALIZA.
        controllers[i]->value =
            ofMap(ofGetMouseX(),
                  controllers[i]->x - (controllers[i]->width * 3 / 4) / 2,
                  controllers[i]->x + (controllers[i]->width * 3 / 4) / 2, 0.0,
                  1.0, true);
        boxes[openguinumber]->parameters.setFloatValue(controllers[i]->value,
                                                       i);
        boxes[openguinumber]->parameters.setFloatLerpValue(
    ...
    for (int i = 0; i < controllers.size(); i++) {
      if (controllers[i]->overboton_collapse &&
          boxes[openguinumber]->parameters.getType(i) ==
              boxes[openguinumber]->parameters.FLOAT) {
        // CAMBIA MOVTYPE
...
void JPboxgroup::load2(string _dirinput) {
  ...
  // NO SE COMO HACERLO EN UNA SOLA PASADA PERO EN 2 RE FUNCA ASI QUE MIRA QUE
  // PIOLA EH
  /*string nombre = _dirinput.substr(_dirinput.find_last_of("/\\") + 1,
  _dirinput.size()); nombre = nombre.substr(0, nombre.find(".xml")); cout <<
  "nombre " << nombre << endl;
  */
...
void JPboxgroup::load(string _dirinput) {
  ...
  for (auto &box : boxloader) {
    ...
    if (directory.getValue().find(".frag") != std::string::npos) {
      ...
      bx = new JPbox_preset();
    } else if (directory.getValue().find("framedifference") !=
               std::string::npos) {
    ...
    for (auto &param : parameters) {
      ...
      if (bx->parameters.getType(index) == bx->parameters.FLOAT) {
        ...
        bx->parameters.setFloatLerpValue(
            param.getChild("value").getFloatValue(), index);
        bx->parameters.setFloatValue(param.getChild("value").getFloatValue(),
                                     index);
        bx->parameters.setmovetype(param.getChild("movtype").getIntValue(),
                                   index);
        bx->parameters.setSpeed(param.getChild("speed").getFloatValue(), index);
      } else if (bx->parameters.getType(index) == bx->parameters.BOOL) {
        bx->parameters.setName(param.getChild("name").getValue());
        bx->parameters.setBoolValue(param.getChild("value").getBoolValue(),
                                    index);
      }
      index++;
    }
    ...
    for (auto &fbolink : fboslinks) {
      for (int i = 0; i < boxes.size(); i++) {
        ...
        if (boxes[i]->name == fbolink.getValue() && i != index1) {
          ...
          // aca tendria que comparar que tipo si lee que tiene un link el
          // shader tendria que tener esa misma cantidad de entradas porque si
          // tipo modificas el shader y le sacas un buffer e intentas levantar
          // un archivo de guardado que tiene un buffer crashea entonces le
          // pongo lo de > 0 pero en realidad tendría que ser que
          if (existe && boxes[index1]->fbohandlergroup.getSize() > 0) {
            boxes[index1]->fbohandlergroup.setFboPointer(
                fbopointer, fbopointername, index2);
          }
        }
      }
      index2++;
  ...
  *activerender = int(
      ofClamp(xml.getChild("activerender").getIntValue(), 0, boxes.size() - 1));
...
void JPboxgroup::setControllers() {
  ...
  */
  ...
  // FIJATE QUE ESTO SI LA PRIMERA CONDICION NO SE CUMPLE NI EVALUA LA SEGUNDA.
  // PARA PODER EVALUAR LA SEGUNDA LA PRIMERA TIENE QUE SER TRU
  if (boxes[openguinumber]->parameters.getSize() > 0 &&
      boxes[openguinumber]->parameters.getMovType(0) != 0) {
    inspectorwindow_height += inspectorwindow_sepy * 0.5;
  }

  for (int k = 0; k < boxes[openguinumber]->parameters.getSize(); k++) {
    if (boxes[openguinumber]->parameters.getType(k) ==
        boxes[openguinumber]->parameters.FLOAT) {
      ...
      sl->setup(inspectorwindow_x, inspectorwindow_height,
                inspectorwindow_width, complexsliderheight,
    ...
    } else if (boxes[openguinumber]->parameters.getType(k) ==
               boxes[openguinumber]->parameters.BOOL) {
      float complexsliderheight = inspectorwindow_sepy * 1.0;
      JPToogle *toogle = new JPToogle();
      toogle->setParametersPointer(
          boxes[openguinumber]->parameters.getJParameter(k));
      toogle->setup(inspectorwindow_x, inspectorwindow_height, slider_width,
                    slider_height, boxes[openguinumber]->parameters.getName(k),
                    boxes[openguinumber]->parameters.getBoolValue(k));
...
void JPboxgroup::listenToOsc(string _dir, float _val) {
  ...
  // string dir = _dir;
  string shadername =
      _dir.substr(_dir.find_first_of("/") + 1, _dir.find_last_of("/") - 1);
      ...
      for (int k = 0; k < boxes[i]->parameters.getSize(); k++) {
        if (boxes[i]->parameters.getName(k) == parametername) {
          ...
          if (boxes[i]->parameters.getType(k) == boxes[i]->parameters.FLOAT) {
            ...
            // ESTO ES PARA QUE SOLO MODIFIQUE EL VALOR DEL SLIDER SOLO SI ESTA
            // ABIERTO ESE COSO
            if (openguinumber == i) {
              controllers[k]->value =
                  _val; // ACa la cantidad de controllers siempre va a ser igual
                        // a la cantidad de parameters size;
            }
          }
        }
      }
  ...
  if (shadername == "openguinumber") {
    ...
    // NO TENGO NI PUTA IDEA QUE HACES ESTE CODIGO DE ACA :  ONDA . PORQUE SI ES
    // IGUAL A 6 O A / O SEA QUE CARAJO
...
bool JPboxgroup::mouseOverGui() {
  ...
  if (ofGetMouseX() > inspectorwindow_x - inspectorwindow_width / 2 &&
      ofGetMouseX() < inspectorwindow_x + inspectorwindow_width / 2 &&
      ofGetMouseY() > inspectorwindow_y - inspectorwindow_height / 2 &&
      ofGetMouseY() < inspectorwindow_y + inspectorwindow_height / 2) {
...
void JPboxgroup::addBox(string directory, float _x, float _y) {
  ...
  // PIOLA EH
  string nombre =
      directory.substr(directory.find_last_of("/\\") + 1, directory.size());
  /*Formatos de video*/
    ...
    for (int i = 0; i < dir.size(); i++) {
      ...
      if (dir2.isDirectory()) {
        ...
        for (int k = 0; k < dir2.size(); k++) {
          ...
          JPbox_shader test;
          test.setup(*font_p, compofolder_path2, compofolder_name2);
...
void JPboxgroup::deleteSelectedShader() {
  ...
  /*Vamos a dejar esto aca por las dudas, que me reinicie todos los dibujos
  cuando limpio uno. esto es para solucionar el tema ese de que cuando borro un
  fboPointer, en vez de borrarlo es como si me pusiera otro shader como
  fboPointer.  Y si por alguna raz�n le haces un clear a todos los fbos entonces
  es como si reiniciara los punteros dentro del fbohandlergroup.fbos,
  Sin embargo. Si hubiera muchisimas cajitas, asumo que hacerle un clear y un
  allocate a todas las cajas es un proceso sumamente lento. pero es lo mismo que
  hace en el resize as� que no s�, es posible que a futuro tenga que
  solucionarlo. Esta modificaci�n es parte del proceso por encontrar ese bug que
  cada tanto(todav�a no s� porque aparece, y hace que crashee la app, as� que
  medio que estamos como doblecheckeando todo e investigando donde mierda puede
  estar ese bug.
  */

  /*for (int i = boxes.size() - 1; i >= 0; i--) {
          boxes[i]->fbo.clear();
          boxes[i]->fbo.allocate(jp_constants::renderWidth,
  jp_constants::renderHeight);
  }*/

  // ACA HAY QUE AGREGAR QUE SI BORRAS EL ULTIMO SETEE EN EL TRANSITION SHADER
  // EL OTRO BUFFER.
  int index = 0;
  for (int i = 0; i < boxes.size(); i++) {
    if (boxes[i]->mouseOver()) {
      ...
      for (int k = boxes.size() - 1; k >= 0; k--) {
        ...
        for (int l = 0; l < boxes[k]->fbohandlergroup.getSize(); l++) {
          // cout << "NUM: " << l << "  NAME : " <<
          // boxes[k]->fbohandlergroup.getFboName(l) << endl;
          if (boxes[k]->fbohandlergroup.getFboName(l) == boxes[i]->name) {
...
ofTexture *JPboxgroup::getActiveTexture() {
  if (boxes.size() >= 1) {
    ...
    // boxes[*activerender]->shaderrender.fbo.draw(0, 0, ofGetWidth(),
    // ofGetHeight());
  }
  return nullptr;
}
int JPboxgroup::getBoxesSize() { return boxes.size(); }
...
int JPboxgroup::getActiverenderNum() { return *activerender; }
/*ofFbo JPboxgroup::getActiverender() {
        if (boxes.size() >= 1) {
                return boxes[*activerender]->shaderrender.fbo;
                //boxes[*activerender]->shaderrender.fbo.draw(0, 0,
