...
void ofApp::keyPressed(int key) {
	...
	if(pantallaActiva == NODOS){
		...
		if (key == 'o') {
			...
			//ESTO ES LO QUE HABRIA QUE PROBAR EN MAC PARA VER SI FUNCA O NO . CUANDO ESTEMOS AHÏ
		...
		if (key == 'm') {
			...
			*/
			string txtname = "-" + ofToString(ofGetDay())+"-"
...
void ofApp::windowResized(int w, int h) {
	...
	//El resize lo hace solo para mover la interfaz. Los tamaños de render se mantienen igual
...
void ofApp::dragEvent(ofDragInfo dragInfo){
	...
	//ESTO TIENE QUE COINCIDIR CON LOS TAMAÑOS DE LAS CAJAS QUE ACTUALMENTE ESTA EN 80x80
	float sepx = 80 * 1.4;
	float sepy = 80 * 1.6;
