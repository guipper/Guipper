...
void JPboxgroup::update() {
	...
	for (int i = boxes.size() - 1; i >= 0; i--) {
		...
		//SE ACTUALICE 
		if (boxes[i]->getTipo() == boxes[i]->VIDEOBOX
			&& openguinumber == i &&
		...
		//PARA QUE RECARGUE EL SHADER AUTOMATICAMENTE PAPÁ.
...
void JPboxgroup::update_mouseDragged(int mousebutton) {
	...
	///				float slidervaluewidth = width * 3 / 4;
					...
					//Esta es la actualización manual. Acordate que si el movtype esta en 0 el valor NO SE ACTUALIZA.
		...
		for (auto& fbolink : fboslinks) {
			for (int i = 0; i < boxes.size(); i++) {
				if (boxes[i]->name == fbolink.getValue() && i != index1) {
					...
					boxes[index1]->fbohandlergroup.
						setFboPointer(fbopointer, fbopointername, index2);
				}
			}
			index2++;
		}
	...
	for (int k = 0; k <boxes[openguinumber]->parameters.getSize(); k++) {
		if (boxes[openguinumber]->parameters.getType(k) == boxes[openguinumber]->parameters.FLOAT) {
			...
			toogle->setup(inspectorwindow_x,
				...
				, slider_width
				, slider_height
				, boxes[openguinumber]->parameters.getName(k)
...
void JPboxgroup::deleteSelectedShader() {
	...
	si me pusiera otro shader como fboPointer.  Y si por alguna razón le haces un clear a todos los fbos entonces
	...
	es un proceso sumamente lento. pero es lo mismo que hace en el resize así que no sé, es posible que a futuro
	tenga que solucionarlo. Esta modificación es parte del proceso por encontrar ese bug que cada tanto(todavía 
	no sé porque aparece, y hace que crashee la app, así que medio que estamos como doblecheckeando todo e investigando
