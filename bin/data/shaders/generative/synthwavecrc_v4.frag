...
const int FSAA = 4;
const float GRID_DIM = 0.67;
const bool JUMP = false;


const vec3 col1 = vec3(0.0,0.3,.2)*.2;
...
void rayMarcher(in vec3 cameraPos, in vec3 lookDir, in vec2 screenDim, in vec2 uv,
    ...
    if(dist <= EPS) {
        ...
        vec3 col_dark = mix(vec3(0.1, 0.4, 0.3), vec3(0.1, 0.4, 0.3), col_t);
        vec3 col_bright = mix(vec3(0., 0.9, 0.), vec3(0., 0.9, 0.), col_t);
        vec3 lightDir = vec3(0.1, -0.4, 1);
        ...
        col = mix(new_col, col, smoothstep(20.,950., cameraPos.z - pos.z)) ;
    }
}
...
vec4 sampleColor(in vec2 sampleCoord )
{
    ...
    col = mix(vec3(165./255.,250./255.,92./255.)*.01,generateStarnest(),.5);
    // Sun
    ...
    // Output to screen
    return vec4(col+g*g*.0000016*vec3(165./255.,250./255.,92./255.), 1.0);
}
...
void main() {
	...
	/*colSum.rg *=rotate2d(0.59*TWO_PI); 
	colSum.bg *=rotate2d(0.34*TWO_PI); 
	colSum.rb *=rotate2d(0.65*TWO_PI); */
	
	vec4 fin = colSum / colSum.w*4.71;
	fin = mix(fin,mix(fin,img,.7),img.a);
	
	fin = mix(fin,mix(fin,img2,.6),img2.a);
    fragColor = fin;
}