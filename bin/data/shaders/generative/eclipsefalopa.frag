...
float ksetmoon(vec3 p) {
    p = (p * .4 + vec3(.14, .31, .51)) * 1.5 + lunasurface;
    float l = 1.;
    float ln = 0.;
    float lnprev = 0.;
    float expsmooth = 0.;
...
float de(vec3 p) {
    ...
    p -= v1;
    luncol = 1. - k * 0.15;
    float sph1 = length(p + v1) - .9 + k * .5 * (lunaextrude - .5);
    float sph2 = length(p + v1) - lunawater * 2. - length(sin(p * 5. + t * 5.)) * .05;
    float d = min(sph1, sph2);
    water = step(sph2, d);
    return d;
}
