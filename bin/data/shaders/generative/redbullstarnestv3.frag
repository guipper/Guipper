...
vec3 generateRedbullBackground(vec2 uv) {
	...
	float e = fbm2(vec2(uv.x * 5., uv.y * 5. + time * .1), time * manimationspeed1 + 1.0);
		 // e*=uv.y*0.4;
    // e=0.5;  
	float e2 = fbm2(vec2(uv.x * .5, uv.y * 5.5), time * 3.0 + 3213.0 + fbmseed * 10.);

	float e3 = fbm2(vec2(uv.x * 10., uv.y * 10.), 0.5 + time * 1.0 + 4123.0 + fbmseed * 10.);

	float e4 = fbm2(vec2(uv.x * mapr(fbmsc, 4.0, 22.0) * .5, uv.y * mapr(fbmsc, 4.0, 22.0)), time * 0.1 + 32.0 + fbmseed * 10.);
