...
void main(void){   
    ...
    float lines = sin(uv2.y*50.-uv2.x*20)*4.5*cir1+0.5;
    //lines+=sin(lines*10+time)*0.2;
    float rx = rxr2(vec2(uv2.x*0.5+time*0.01,uv2.y*0.5))*lines;
    ...
    vec3 dib2 = vec3(1.0,1.0,1.0);
    
    
    vec3 col3 = vec3(1.0,1.0,0.0);
    vec3 col4 = vec3(0.6,0.2,0.3);
    
    
    float n = snoise(vec2(uv2.x*2.,uv2.y*10.))*0.2+0.0;
    dib2 = col4 * vec3(sin(rx*2.)*.9)*sm(0.1,1.0,lines);
