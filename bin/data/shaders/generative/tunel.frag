...
float de(vec3 p) 
{
    ...
    radialCopy(p2.xy, 12.+sin(time*.1)*5., mapr(radial2,2.0,5.)); // copiar radialmente
...
vec3 shade(vec3 p, vec3 dir)
{
    ...
    vec3 n = normal(p);
    vec3 dif1 = max(0., dot(lightdir1, n)) * light1color * fade1 * 0.9;
    vec3 dif2 = max(0., dot(lightdir2, n)) * light2color * fade2 * 0.9;
...
void main(void)
{
    ...
    lightpos1 = path(t + 5. * (1. + sin(time*movspeed / 2.)*.5)) + vec3(sin(time*movspeed) * 0.5, 0.0, 5.); 
    lightpos2 = path(t + 15.) + vec3(-sin(time*movspeed) * 1., 0., 1.);
