...
float desf(vec2 uv, float _fase) {
	...
	e = sin(uv.y * mapr(v1, 0., 50.) + mt + _fase + sin(uv.y * mapr(v2, 0., 50.) + mt + _fase + sin(uv.x * mapr(v3, 0., 50.) + mt + _fase + sin(uv.y * mapr(v4, 0., 50.) + mt + _fase) * mv4_amp * 0.5 + mv4_amp * 0.5) * mv3_amp * 0.5 + mv3_amp * 0.5) * mv2_amp * 0.5 + mv2_amp * 0.5) * mv1_amp * 0.5 + mv1_amp * 0.5;
