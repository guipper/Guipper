...
vec3 tex(vec2 p) {
    ...
    if(mirror)
        p = abs(.5 - fract(p * texturesize * 2.));
    else
        p = abs(fract(p * texturesize * 2. + .5));
...
float de(vec3 p) {
    ...
    if(i == 0)
        d = sdSphere(p, 3. + t_e * deftext);
    if(i == 1)
        d = sdBox(p, vec3(2. + t_e * deftext));
    if(i == 2)
        d = sdTorus(p, vec2(2., 1. + t_e * deftext));
    if(i == 3)
        d = sdHexPrism(p, vec2(2., 1. + t_e * deftext));
    if(i == 4)
        d = sdOctahedron(p, 3. + t_e * deftext);
    return d;
}
