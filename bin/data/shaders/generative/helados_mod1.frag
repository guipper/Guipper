...
float de(vec3 p)
{
    ...
    float box = 2;
    vec3 pos = p;
    float pz = abs(fract(sin(p.z*0.33+time)*1.0-1));
    ...
    for(int i=0; i<cnt; i++){
      ...
      //    p.z+= sin(index*pi*+time);
        ...
        pcono.x+= sin(pcono.y*10.*freqx+time*2+sin(p.z*2.)*10.)*ampx;

		
		//pcono = opElongate(p,vec3(0.2));
        float mof = cos(sin(p.z*100.)*0.5+index*pi*4.+time)*defor*0.08;
  ...
  //  if (d==box) objcol=vec3(0.8-pz,sin(length(fract(pos.zz))*1)*0.7,pz);
    ...
    if (d==pla) objcol=vec3(0.1,0.8,0.7);
...
vec3 shade(vec3 p, vec3 dir) {
    ...
    return col*(amb*10.0 + diff+flmod) + spec * 0.1;
