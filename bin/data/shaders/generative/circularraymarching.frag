...
float mapr2(float _value,float _minin,float _maxin, float _low2,float _high2) {
	
	float val = _low2 + (_high2 - _low2) * (_value - _minin) / (_maxin - _minin);
    //float val = 0.1;
	return val;
}
float de(vec3 p) 
{
	...
	//psph.x+=sin(time)*5.5;
	float fz = 2.;
	float tilez = mapr(ztile,1.0,30.0);
	
	
	
	//psph.x+=5;
	float indexx = floor(psph.z/fz);
	
	psph.z = abs(mod(psph.z,tilez))-tilez/2.;
	
	psph.xy*=rotate2d(indexx*fas*10.1);
	//psph.x+=5;
	//radialCopy(psph.xy,20.0,19.);
	
	//psph.x-=1.;
	float mradial = floor(mapr(radialtile,2.0,100.0));
	radialCopy(psph.xy,mradial,4.);
	
	
	float def = length(psph);
	
	
	vec2 uv3 = gl_FragCoord.xy / resolution;
	vec2 p2d = vec2(0.5) - uv3;
	float r2d = length(p2d);
	
	float def2 = 0.5*size+sin(r2d*50.+time+p.y*4.)*.1;
	float sph = sphere(psph, size*5.5);
	...
	//objcol = mix(vec3(1.0,0.0,0.0),vec3(1.0,1.0,.0),indexx);
	
	
	vec3 objcol1 = vec3(rdm(5.+indexx),rdm(23.+indexx),rdm(432.+indexx));
	vec3 objcol2 = vec3(rdm(521.+indexx),rdm(4.+indexx),rdm(612.+indexx));
