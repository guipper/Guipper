...
void main() {
	...
	for(int i = 1; i < mcnt; i++) {
		...
		float e4 = sin(uv.y * 5. + golpe * 10. + time + sin(uv.x * 10.)) * mapr(amp2, 0.0, 0.4);

		uv2 += random2(uv2 * uv.x * 400. + sin(uv.y * 100. + time) * 0.1) * e4 * 4.;
		...
		col2 = vec3(sin(e * 100. + time) * 0.5, sin(e * 10.) * 1.5, 0.0);

		dib += vec3(e) * mix(col2, col1, e) * brillo * 4.;
	}
