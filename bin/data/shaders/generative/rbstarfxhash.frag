...
vec3 generateRedbullBackground(vec2 uv) {
	...
	float e = fbm2(vec2(uv.x * 5., uv.y * 5. + time * .1), time * manimationspeed1 + 1.0);
		 // e*=uv.y*0.4;
    // e=0.5;  
	float e2 = fbm2(vec2(uv.x * 5.5, uv.y * 5.5), time * 3.0 + 3213.0 + fbmseed * 10.);

	float e3 = fbm2(vec2(uv.x * 10., uv.y * 10.), 0.5 + time * 1.0 + 4123.0 + fbmseed * 10.);

	float e4 = fbm2(vec2(uv.x * mapr(fbmsc, 4.0, 22.0), uv.y * mapr(fbmsc, 4.0, 22.0)), time * 0.1 + 32.0 + fbmseed * 10.);

	float e5 = fbm2(vec2(uv.x * 2., uv.y * 2.), 0.5 + time * 0.2 + 31234.0);
...
//	fin = smoothstep(0.75,1.0,fin);
	...
	fin = mix(fin, mix(c3, c4, vec3(e4)), smoothstep(0.25, .99 + e * 0.1, 1. - uv.y)); //COn un toque de blanco
