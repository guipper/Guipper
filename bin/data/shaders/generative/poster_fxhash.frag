...
void main(void){   
  ...
  //  float n = snoise(vec2(uv.x*100.*noisex,uv.y*100.*noisey+time*1)*0.002) ;
	...
	float msc2 = mapr(sc2,0.1,1.);
	
	float mseed = seed*10000.;
//	float flush
    float e = ridgedMF2(vec2(uv.x*0.5*uv.y,uv.y*0.5)
             *(ridgedMF2(vec2(uv.x*3.2*msc2+flush*50.,uv.y*10.0*msc2+time*0.04)))
             *ridgedMF2(vec2(uv.x*1*msc2,uv.y-time*0.002+seed*10.)*ridgedMF2(vec2(uv.x*0.5,uv.y*0.5-time*0.002))))
             *sin(uv.x*msc+fasex*4.0)*sin(uv.y*msc+fasey*4.0)
             ;
