...
float noiseW(in vec2 p) {
	...
	return -1.0 + 2.0 * mix(mix(hashW(i + vec2(0.0, 0.0)), hashW(i + vec2(1.0, 0.0)), u.x), mix(hashW(i + vec2(0.0, 1.0)), hashW(i + vec2(1.0, 1.0)), u.x), u.y);
}
...
float de(vec3 p) {
	float freq = SEA_FREQ;
	float amp = SEA_HEIGHT + turbina * .3 * (1. - smoothstep(0., 2., abs(p.x)));
	...
	p.y += tu;
	p.y -= smoothstep(0., 3., p.z - 3.) * tu * .7;

	float dd = -p.y + 2. - h - espuma(-p.xz) * .5 * turbina;
	p.y -= 2.;
	p.z -= 10.;
	return dd;
}
...
vec3 march(vec3 from, vec3 dir) {
	...
	if(d < det) {
		...
		float e = espuma(-p.xz);
		col = mix(col, e * vec3(.5, .6, .7), e * turbina);
//	col+=e*turbina*vec3(.7);
	} else {
		col = back;
	}
	return col + g * g * .002 * (1. - smoothstep(1., 4., abs(p.x))) * turbina;
}
