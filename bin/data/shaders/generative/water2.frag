...
float de(vec3 p) {
	...
	float tu = smoothstep(0., 1., abs(p.x)) * 2. * turbina - turbina * 2.;
	p.y += tu;
...
vec3 shade(vec3 p, vec3 dir, vec3 n) {
	...
	return (SEA_WATER_COLOR + noise(p.xz) * .3) * dif + spe * brilloagua;
}
...
vec3 march(vec3 from, vec3 dir) {
	...
	if(d < det) {
		...
		float e = (espuma(-p.xz) * brilloespuma + glow * gl * .5 * brilloespuma) * (.5 + .5 * smoothstep(0., 5., p.z + 10.));
