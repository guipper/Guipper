...
vec3 generateRedbullBackground(vec2 uv) {
	...
	float mapspeedx = mapr(speedx, -.01, .01);
	float mapspeedy = mapr(speedy, -.01, .01);
...
//	fin = smoothstep(0.75,1.0,fin);
	...
	fin = mix(fin, vec3(e4), smoothstep(0.1, .99, 1. - uv.y)); //COn un toque de blanco
...
vec3 generateStarnest() {
	...
	for(int r = 0; r < mvolsteps; r++) {
		...
		for(int i = 0; i < mite; i++) { 
			...
			p = abs(p) / clamp(abs(p.x) * abs(p.y), .005, 10.5) - mapr(formuparam, 0.2, 0.4);			// the magic formula
