...
float vertex_height(vec2 vf) {
    float offset = 0.5 + 2.0*smoothstep(1., 5., abs(vf.x));
    float amplitude = mix(0.0, 2.5, smoothstep(1., 8., abs(vf.x)));
	amplitude = 0.0;
	offset = 0.2;
...
float sdf(in vec3 pos ) {    
	...
	float s = pos.y*2.3 - height_and_color_map(pos.xz)*2.;
	s+=smoothstep(0.1,0.2,pos.x*5.)*5.;
	s+=(1.-smoothstep(0.1,0.2,pos.x*5.))*5.;
...
void rayMarcher(in vec3 cameraPos, in vec3 lookDir, in vec2 screenDim, in vec2 uv,
    ...
    if(dist <= EPS) {
        ...
        float col_t = smoothstep(4., 5., abs(pos.x));
        //vec3 col_dark = mix(vec3(0.1, 0.5, 0.3), vec3(0.1, 0.4, 0.3), col_t);
        //vec3 col_bright = mix(vec3(0., 0.2, 0.), vec3(0., 0.9, 0.), col_t);
        
		vec3 col_dark = mix(vec3(0., 85.0/255., 88.0/255.), vec3(0., 85.0/255., 88.0/255.), col_t);
        vec3 col_bright = mix(vec3(0., 0.9, 0.)*4., vec3(0., 0.9, 0.)*.3, col_t);
		...
		float offset = -1.0; // Puedes ajustar este valor para mover las líneas hacia el centro o los extremos

		float posx2 = pos.x+0.5;
		float posx3 = pos.x-0.9;
		float linea1 = smoothstep(6.5 + offset, 8.0 + offset,posx2) * smoothstep(7.5, 6.5, posx2) * dx;
		float linea2 = smoothstep(6.5 + offset, 8.0 + offset, 1.0 - posx3) * smoothstep(7.0, 6.5, 1.0 -posx3) * dx;
		
		
		
		
		vec3 c1 = vec3(0.0,0.9,0.05);
		vec3 linea1g = g*g*0.0001*linea1*c1;
		vec3 linea2g = g*g*0.0001*linea2*c1;
		//new_col*=3.5;
		...
		col = mix(new_col,
				  col, smoothstep(10.,950., cameraPos.z - pos.z)) ;
		

		vec3 lg = +g*g*0.00015*linea2*c1
					+g*g*0.0001*linea1*c1;
		//col = mix(col,linea1g,1.0);
		/*col = mix(new_col,
				  col, smoothstep(10.,950., cameraPos.z - pos.z)) ;
		col+= mix(col*lg,new_col*10.5,lg);
...
vec4 sampleColor(in vec2 sampleCoord )
{
    ...
    col = mix(vec3(165./255.,250./255.,92./255.)*.01,generateStarnest(),1.-col);
	...
	return vec4(col+g*g*.00001*(vec3(165./255.,220./255.,92./255.)*0.2), 1.0);
}
...
void main() {
	...
	/*fin = mix(fin,mix(fin,img,1.0),img.a);
	fin = mix(fin,mix(fin,img2,0.9	),img2.a*alphalogos2);
