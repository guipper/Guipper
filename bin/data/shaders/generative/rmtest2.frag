...
float de(vec3 p) 
{
	...
	float def = length(sin(p*5.+time*2.))*.2;
		  def = snoise(p.xy+time)*10.;
    float sph = sphere(psph, 1.3+def);
    ...
    if (d == sph) objcol = mix(vec3(1.0,0.0,0.0),vec3(.0,0.0,1.0),smoothstep(0.1,0.5,def));
