...
void main()
{
  ...
  vec2 ao = vec2(0);
  for (int i = 0; i < 8; i++) {
    ivec2 id = (ivec2(gl_FragCoord.xy/16.+vec2(iTime*10.,iTime*20.)) % 2)*2-1;
    ao += vec2(march(p+n*0.1, r, bayer(ivec2(gl_FragCoord.xy)+i+ivec2(i/4,0))) , 1.);
  }
  ao.x/=ao.y;

  fragColor = hit&&!outline ? vec4(vec3(ao.x),1.0) : vec4(vec3(0.),1.0);
  fragColor = vec4(vec3(pow(smoothstep(vec3(0),vec3(1),sqrt(fragColor.xyz)),vec3(1.7,1.6,1.5))),1.0);
