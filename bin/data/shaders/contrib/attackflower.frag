...
#if STOP_MOTION
  float framy = 0.15;
  float t2 = floor(time*speed*4./framy)*framy;
#else
  float t2 = time*speed*4.;
#endif
...
void cam(inout vec3 p) {
  ...
  p.yz *= rot(0.3 + sin(time*speed*4.*0.3)*0.17);
  p.xz *= rot(time*speed*4.*0.2 + curve(time*speed*4., 5.2)*2.0);
}
...
void main()
{
  ...
  vec3 s = vec3(0,0,-50.0 - curve(time*speed*4., 2.3)*10.0);
  vec3 r = normalize(vec3(-uv, 0.5+0.3*curve(time*speed*4., 1.3)));
  ...
  //col = vec3(noise(vec3(uv*50,time*speed*4.)));
