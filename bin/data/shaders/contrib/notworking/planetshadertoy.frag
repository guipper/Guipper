...
float fbm2( const in vec3 p, const in float a, const in float f) {
    float ret = 0.0;    
    float amp = 1.0;
    float frq = 1.0;
    for(int i = 0; i < fbm2_STEPS; i++) {
        float n = pow(noise2(p * frq),2.0);
        ret += n * amp;
        frq *= f;
        amp *= a * (pow(n,0.2));
    }
...
float asteroidMap( const in vec3 p, const in vec3 id) {
    float d = asteroidRock(p, id) + noise2(p*4.0) * ASTEROID_DISPLACEMENT;
    return d;
}

float asteroidMapDetailed( const in vec3 p, const in vec3 id) {
    float d = asteroidRock(p, id) + fbm2(p*4.0,0.4,2.96) * ASTEROID_DISPLACEMENT;
    return d;
}
...
float renderRingFarShadow( const in vec3 ro, const in vec3 rd ) {
    ...
    if( d > 0. ) {
        ...
        if( l > RING_INNER_RADIUS && l < RING_OUTER_RADIUS ) {
            return .5 + .5 * (.2+.8*noise2( l*.07 )) * (.5+.5*noise2(intersection.xy));
        } else {
            return 0.;
        }
    } else {
...
vec4 renderRingFar( const in vec3 ro, const in vec3 rd, inout float maxd ) {
    ...
    if( d > 0. && d < maxd ) {
        ...
        if( l > RING_INNER_RADIUS && l < RING_OUTER_RADIUS ) {
            float dens = .5 + .5 * (.2+.8*noise2( l*.07 )) * (.5+.5*noise2(intersection.xy));
            vec3 col = mix( RING_COLOR_1, RING_COLOR_2, abs( noise2(l*0.2) ) ) * abs(dens) * 1.5;
...
vec4 renderRing( const in vec3 ro, const in vec3 rd, inout float maxd ) {
    ...
    if( abs(ro.z) < RING_HEIGHT+RING_DETAIL_DISTANCE 
	    ...
	    float d = iPlane( ro, rd, vec4( 0., 0., 1., 0.) );
        float detail = mix( .5 * noise2( fract(ro.xy+rd.xy*d) * 92.1)+.25, 1., smoothstep( 0.,RING_DETAIL_DISTANCE, d) );
        far.xyz *= detail;    
    }
...
vec3 terrainGetColor( const in vec3 pos, const in vec3 rd, const in float t, const in vec3 sunc, const in vec3 upc, const in vec3 reflc ) {
    ...
    float no = noise2(5.*fbm2(1.11*pos.xz));
#else
    const float no = 0.;
#endif
    float r = .5+.5*fbm2(.95*pos.xz);
	vec3 col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.07,0.07), vec3(0.10,0.09,0.08), noise2(0.4267*vec2(pos.x*2.,pos.y*9.8))+.01*no );
    ...
    float s = nor.y + 0.03*pos.y + 0.35*fbm2(0.05*pos.xz) - .35;
...
vec4 renderClouds( const in vec3 ro, const in vec3 rd, const in float d, const in vec3 n, const in float land, 
    ...
    float rot = -.2*length(cint.xy) + .6*fbm2( cint*.4,0.5,2.96 ) + .05*land;
    ...
    float clouds = 1.3*(fbm2( cint*(1.+.02*noise2(intersection)),0.5,2.96)+.4*land-.3);

#ifdef DISPLAY_CLOUDS_DETAIL
    if( d < 200. ) {
        clouds += .3*(fbm2(cdetail,0.5,2.96)-.5)*(1.-smoothstep(0.,200.,d));
    }
#endif
...
vec4 renderPlanet( const in vec3 ro, const in vec3 rd, const in vec3 up, inout float maxd ) {
    ...
    if( renderSeaDetail ) {    
        ...
        e = fbm2( .003*intersection+vec3(1.),0.4,2.96) + smoothstep(.85,.95, abs(intersection.z/EARTH_RADIUS));
#ifndef HIDE_TERRAIN
        if( d < 1500. ) {
            e += (-.03+.06* fbm2( intersection*0.1,0.4,2.96))*(1.-d/1500.);
        }
#endif  
    }
    ...
    if( mixDetailColor < 1. ) {
        if( e < .45 ) {
            ...
            // planet (land) far
            float land1 = max(0.1, fbm2( intersection*0.0013,0.4,2.96) );
            float land2 = max(0.1, fbm2( intersection*0.0063,0.4,2.96) );
...
#ifdef HIGH_QUALITY
            col *= (.5+.5*fbm2( intersection*0.23,0.4,2.96) );
#endif
        }
    }
...
// black bands
    ...
    if( uv.y < bandy.x || uv.y > bandy.y ) {
        ...
        if( lro < EARTH_RADIUS+EARTH_CLOUDS*1.25 ) {
            ...
            col = mix( col, .5*sunColor, .15*abs(noise2(offset*100.))*clamp(1.-4.*abs(offset)/EARTH_CLOUDS, 0., 1.) );
        }
#endif 
