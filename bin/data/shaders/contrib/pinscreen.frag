...
/*************************************\
 ...
 P.S.: I wonder if I'm currently holding the record for the total amount of time2s
...
float ao(vec3 p, vec3 n) {
    ...
    for (float depth = 0.05; depth <= 0.25; depth += 0.05) {
        vec3 dir = normalize(n + randomPointOnHemisphere(n, i + depth * 10.0 + hash(p) + time2));
...
vec3 getEye() {
    vec3 eye1 = rotY(0.65 + sinf(1.0 * time2 * TAU * 0.02) * 0.5) * vec3(-3.0, 7.0, -4.0);
