...
void main() {
	...
	vec3 coluser = vec3(r,g,b);
	if(cam < 0.25){
		camPos = vec3(5.0+12.0*sin(t*0.05),5.0+ 7.0*cos(t*0.05), 1.9);
		camTarget = vec3(5.0+0.0,5.0+7.0*sin(t*0.5), 0.0);
	}else if(cam < 0.5){
	  ...
	  camPos = vec3(5.*t,3.1*t,2.1);
	camTarget = vec3(5.*tt,3.1*tt,1.0);
