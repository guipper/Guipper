...
vec2 map( vec3 q )
{
    ...
    float r = 15.0;
    q.y -= r;
    float ani = pow( 0.5+0.5*sin(6.28318*iTime*speedlatido*5.0 + q.y/25.0), 4.0 );
...
vec3 render( in vec2 p )
{
	...
	float an = 0.2*(iTime*0.0+2.0);

	vec3 ro = vec3(0.4*sin(an),mapr(rotY,0.06,1.0),0.4*cos(an));
