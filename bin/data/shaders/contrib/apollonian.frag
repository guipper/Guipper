...
void main()
{
    ...
    {	
        ...
        vec2 q = gl_FragCoord.xy.xy+vec2(float(ii),float(jj))/float(AA);
        vec2 p = (2.0*q-iResolution.xy)/iResolution.y;

        // camera
        vec3 ro = vec3( 2.8*cos(0.1+.33*mtime), 0.4 + 0.30*cos(0.37*mtime), 2.8*cos(0.5+0.35*mtime) );
        vec3 ta = vec3( 1.9*cos(1.2+.41*mtime), 0.4 + 0.10*cos(0.27*mtime), 1.9*cos(2.0+0.38*mtime) );
        float roll = 0.2*cos(0.1*mtime);
