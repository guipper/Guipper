...
//https://www.shadertoy.com/view/Mddczn
...
void main()
{
  ...
  snowFac = SmoothBump (0.4, 0.9, 0.05, mod (0.8 * tCur / tCyc, 1.));
  vel = 0.02;
  vuPos.xz = TrackPathS (vel * tCur);
  //vuPos.y = 0.1;
  ro = vuPos;
  vd = TrackPathS (vel * (tCur + 0.8)) - vuPos.xz;
  az = atan (vd.x, vd.y);
  el = 0.;
  if (mPtr.z > 0.) {
    az += 2. * pi * mPtr.x;
  ...
  rd = vuMat * normalize (vec3 (uv,1.0));
  dstFar = 80.;
