...
float de(vec3 p) 
{
	...
	psph.xy*=rotate2d(indexx+time*.2);
	//psph.x+=5;
	//radialCopy(psph.xy,20.0,19.);
	
	//psph.x-=1.;
	radialCopy(psph.xy,15.0,4.);
	
	float def = length(psph);
	float sph = sphere(psph, 1.3*size+sin(p.x*3.+time+p.y*4.)*.1);
	...
	objcol = vec3(rdm(5.+indexx),rdm(23.+indexx),rdm(432.+indexx));
