...
float obj2(vec3 p){
	...
	vec3 sphcol = vec3(sin(p.y*1.)*.5+.5,sin(p.x*1.)*.5+.5,sin(p.z*1.)*.5+.5);
	objcol = mix(vec3(1.0,1.0,0.0),sphcol,box);
	objcol = mix(vec3(1.0,0.0,1.0),objcol,line);
...
{	
	...
	o1 = opSmoothUnion(o1,obj2(p2),1.0);
	
	objcol = mix(vec3(1.0,0.0,0.0),vec3(0.0,0.0,1.0),obj2(p2));
	return o1;
}
...
vec3 march(vec3 from, vec3 dir) 
{
	...
	p -= det * dir;
	col = mix(vec3(0.0),mix(shade(p, dir),vec3(0.0),0.0),d<det);
