#!/usr/bin/env python3
"""Run the native legacy composition loading regression without changing user sessions or profiles.

Linux: xvfb-run -a python3 tests/run_legacy_composition.py
Windows (with a desktop): python tests/run_legacy_composition.py
"""
import argparse
import os
...
def main():
    ...
    parser.add_argument('--log', type=Path, default=ROOT / 'dist' / 'legacy-composition.log')
    args = parser.parse_args()
    args.log.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='guipper-legacy-composition-') as folder:
        work = Path(folder)
        ...
        for key in list(env):
            if key.startswith('GUIPPER_') or key in ('APPIMAGE', 'APPDIR'):
                env.pop(key)
        env.update(GUIPPER_PERSISTENCE_TEST='legacy_composition', GUIPPER_USER_ROOT=str(work / 'profile'))
        ...
        for line in text.splitlines():
            if 'legacy-composition' in line:
                print(line)
        passed = result.returncode == 0 and 'legacy-composition: passed=1' in text
