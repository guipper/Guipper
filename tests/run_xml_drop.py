#!/usr/bin/env python3
"""Run the native XML drag-and-drop regression without changing user sessions or profiles.

Linux: xvfb-run -a python3 tests/run_xml_drop.py
Windows (with a desktop): python tests/run_xml_drop.py
"""
import argparse
import os
...
def main():
    ...
    parser.add_argument('--log', type=Path, default=ROOT / 'dist' / 'xml-drop.log')
    args = parser.parse_args()
    args.log.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='guipper-xml-drop-') as folder:
        work = Path(folder)
        ...
        for key in list(env):
            if key.startswith('GUIPPER_') or key in ('APPIMAGE', 'APPDIR'):
                env.pop(key)
        env.update(GUIPPER_PERSISTENCE_TEST='xml_drop', GUIPPER_USER_ROOT=str(work / 'profile'))
        ...
        for line in text.splitlines():
            if 'xml-drop' in line:
                print(line)
        passed = result.returncode == 0 and 'xml-drop: passed=1' in text
