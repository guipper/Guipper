#!/usr/bin/env python3
"""Run the native CUE media regression without changing user sessions or profiles.

Linux: xvfb-run -a python3 tests/run_cue_media.py
Windows (with a desktop): python tests/run_cue_media.py
"""
import argparse
import os
...
def main():
    ...
    parser.add_argument('--log', type=Path, default=ROOT / 'dist' / 'cue-media.log')
    args = parser.parse_args()
    args.log.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='guipper-cue-media-') as folder:
        work = Path(folder)
        ...
        for key in list(env):
            if key.startswith('GUIPPER_') or key in ('APPIMAGE', 'APPDIR'):
                env.pop(key)
        env.update(GUIPPER_PERSISTENCE_TEST='cue_media', GUIPPER_USER_ROOT=str(work / 'profile'))
        ...
        for line in text.splitlines():
            if 'cue-media' in line:
                print(line)
        passed = result.returncode == 0 and 'cue-media: passed=1' in text
